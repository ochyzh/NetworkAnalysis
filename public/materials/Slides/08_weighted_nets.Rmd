...
- For the sample space of valued ERGMs, we need to define $\mathcal{S}$, the set of possible values each relationship may take. For example, for count data, that’s $\mathcal{S}=\{0,1,…,s\}$ if the maximum count is $s$ and $\{0,1,…\}$ if there is no a priori upper bound. Having specified that, $\mathcal{Y}$ is defined as some subset of $\mathcal{S}^\mathcal{Y}$: the set of possible ways to assign to each relationship a value.
---
...
#How prevalent are k-stars in the observed network?
obsNet<-summary(net~sum+transitiveweights("min","max","min")+nodesqrtcovar(TRUE), response = "coopActions")
...
```{r, echo=FALSE, eval=TRUE, message=FALSE, fig.width=10, fig.height=5.5,fig.align="center"}
load("data/val_ergms1.Rdata")
#mcmc.diagnostics(m2)
```
