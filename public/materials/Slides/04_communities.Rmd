...
- A community is a group whose members interact much more with each other than with outsiders.

- More formally: a community is a locally dense connected subgraph. (Barabasi, 2016, 325)
...
# Two groups of six nodes
group1 <- paste0("A", 1:6)
group2 <- paste0("B", 1:6)

# Create dense connections within each group
edges_group1 <- combn(group1, 2)
edges_group2 <- combn(group2, 2)

# Add one bridge between the groups
bridge <- matrix(c("A6", "B1"), nrow = 2)

# Combine all edges
el <- t(cbind(edges_group1, edges_group2, bridge))

# Create undirected graph
g <- graph_from_edgelist(el, directed = FALSE)

# Assign group membership for plotting
V(g)$community <- ifelse(grepl("^A", V(g)$name), 1, 2)

# Place the two groups apart from one another
set.seed(123)

layout_left <- layout_in_circle(induced_subgraph(g, group1))
layout_right <- layout_in_circle(induced_subgraph(g, group2))

layout_left[, 1] <- layout_left[, 1] - 2
layout_right[, 1] <- layout_right[, 1] + 2

LO <- rbind(layout_left, layout_right)
...
# More cross-community connections
bridges <- matrix(
  c(
    "A2","B4",
    "A3","B2",
...
el <- t(cbind(edges_group1, edges_group2, t(bridges)))
...
Instead of asking *What are the communities?*, ask *Which partition best captures the structure of the network?*

Different algorithms define "best" differently.
...
Therefore: different algorithms may return different—but equally reasonable—community assignments.
