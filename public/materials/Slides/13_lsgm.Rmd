...
# From binary outcomes to network edges

The dependent variable can be an edge: present or absent.
...
---
## Potential edges become observations

- The rows of the data are potential edge-pairs, not actors.

| edge_id | Y | trade | mil_ratio |
|:--------|--:|------:|----------:|
| 1–2     | 1 |  0.36 |       1.4 |
| 1–3     | 0 |  0.12 |       0.8 |
| 1–4     | 0 |  0.49 |       2.2 |
...
$$\text{logit}(\kappa_i)=X_i^T\beta$$
$\kappa_i$ is the baseline edge probability implied by exogenous covariates. It is what a standard logit would estimate before local dependence enters.

- The LSGM then asks whether nearby edges push Yᵢ above or below this baseline.

---
## Define a connectivity matrix W among potential edges
.pull-left[
```{r, echo=F, out.width= "500px",fig.align="center"}
knitr::include_graphics("./images/W.png")
```
]

.pull-right[
- $W_{ij}$ means edge $j$ is a neighbor of edge $i$.

- $W$ can also be continuous, such as geographic distance, shared-node similarity, or theoretical relatedness.

]
---
## Local Markov assumption

- The ordinary logit model would treat each potential edge as conditionally independent given covariates.
- But many network theories say that edges react to other edges.

- Condition on neighbors rather than the entire network

$$\begin{align}
f(y(s_i)|{\textbf{y}(s_j):s_j \neq s_i})=f(y(s_i)|\textbf{y}(N_i))
\end{align}$$

- Nᵢ is the set of potential edges connected to edge i by W.
- The conditional model is local: only neighbors enter directly.
- If W is continuous, the Markov language is less important; all edges can contribute with different weights.
...
- $A_i$ is the log-odds of edge $i$, conditional on its local environment.
---
...
```{r, echo=FALSE, eval=TRUE, message=FALSE, out.width= "800px",fig.align="center"}
knitr::include_graphics("images/eta.png")
```

- Neighboring edges can be complements or substitutes.

- Centering on κⱼ means the dependence term asks whether neighbors are more or less active than their own baseline probabilities.

---
## Logit to LSGM

```{r, echo=FALSE, eval=TRUE, message=FALSE, out.width= "800px",fig.align="center"}
knitr::include_graphics("images/binary_to_lsgm.png")
```

LSGM is logistic regression for edge indicators when edge probabilities depend on local edge structure.
...
---
## Standard errors by simulation
```{r, echo=FALSE, eval=TRUE, message=FALSE, out.width= "800px",fig.align="center"}
knitr::include_graphics("images/simulate_se.png")
```

- Gibbs idea: update one edge at a time conditional on the current values of its neighbors.

---
## Gibbs update intuition

- At step i: compute pᵢ from current neighbor values, draw Yᵢ ~ Bernoulli(pᵢ), replace the old Yᵢ, then move to i+1.

```{r, echo=FALSE, eval=TRUE, message=FALSE, out.width= "800px",fig.align="center"}
knitr::include_graphics("images/gibbs_intuition.png")
```
...
---
---
## Extending the LSGM Over Time

Motivating example: Russian artillery attacks on Chechen villages
...
Previously, we modeled one binary outcome for each potential edge or spatial location.
We now observe the same set of villages repeatedly over time.

For each date (t), define:
...
where (Y_{it}=1) if village (i) experienced an attack on date (t).

Each date can be viewed as a new realization of the spatial process.
---
## The Model at Time t

First calculate the baseline attack probability:

$$\operatorname{logit}^{-1}
\left(
\mathbf{x}_{it}^{T}\boldsymbol{\beta}
\right)$$

The natural parameter for village (i) on date (t) is:

$$\log\left(
\frac{\kappa_{it}}{1-\kappa_{it}}
\right)
+
\eta
\sum_j
...
$$P(Y_{it}=1\mid\mathbf{Y}_{-i,t})=\frac{\exp(A_{it})}{1+\exp(A_{it})}.$$

The dependence term uses outcomes from other villages on the same date.


---
## Spatial and Temporal Dependence

The model includes two different kinds of dependence.

- Spatial dependence

$$
\eta\mathbf{W}
(\mathbf{Y}_t-\boldsymbol{\kappa}_t)
$$

captures whether attacks in connected villages are associated within date (t).

- Temporal dependence

    - *Rollattk7* captures the village's attack history through an observed covariate.

Thus:

- $\eta$ captures contemporaneous spatial dependence.
- rollattk7 captures temporal persistence.
- The spatial term does not directly connect outcomes from different dates.

---
## Pseudolikelihood Over Time

For one date, the log pseudolikelihood is:
...
With multiple dates, add the contributions from every village and every date:
...
The parameter estimates are shared across dates:

$$\arg\max_{\boldsymbol{\beta},\eta}
\log PL.$$

Each date contributes additional information about the same underlying process.

---
```{r, eval=F, echo=T}
vilmat <- read.csv("./data/vilMat.csv")

# Binary outcome: attack in village i on date t
Y <- chechen_attacks$attack

# Explanatory variables
X <- cbind(
  constant = 1,
  population = chechen_attacks$pop_2002 -
    mean(chechen_attacks$pop_2002, na.rm = TRUE),
  recent_attacks = chechen_attacks$rollattk7
)
...
# Date associated with each village-day observation
dates <- chechen_attacks$date
# Connectivity matrix among villages
W <- as.matrix(sqrt(vilmat) / 10000)

#Because the dependence term is calculated as W %*% (Y_t - kappa_t)

#the relevant diagnostic is the sum of each row of W:

summary(rowSums(W))

#Very large row sums may allow the dependence term to overwhelm the baseline logit component.
```
---
...
3. Your goal is to estimate *attacks* as a function of other contemporaneous attacks, as well as population and a 7-day rolling sum of previous attacks. Notice that you have a temporal component you need to accommodate.
...
vilmat<-read.csv("./data/vilMat.csv")
Y=chechen_attacks$attack
X<-as.matrix(cbind(1,chechen_attacks$pop_2002-mean(chechen_attacks$pop_2002), chechen_attacks$rollattk7))
W<-as.matrix(sqrt(vilmat)/10000)
apply(W,2,sum) #Check if W is overwhelming the likelihood

dates<-chechen_attacks$date


#Likelihood
loglik<-function(par,X,W,Y,dates){
...
X_t<-X[dates==i,]
Y_t<-Y[dates==i]
xbeta_t = as.numeric(X_t %*% betas)
kappa_t<-exp(xbeta_t)/(1+exp(xbeta_t)) #logit of Xb
A_i_t=log(kappa_t/(1-kappa_t))+eta*W%*%(Y_t-kappa_t) #Eqn 2
}
A_i<-c(A_i,A_i_t)
p_i<- exp(A_i)/(1+exp(A_i)) #Eqn 1, also Eqn 4
PL<-Y*log(p_i)+(1-Y)*log(1-p_i) #Eqn 3
...
m1<-optim(par=c(0,0,0,0),loglik,X=X,W=W,Y=Y,dates=dates)
